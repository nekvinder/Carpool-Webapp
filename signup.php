...
$sql = "SELECT * FROM login where uname='" . $_POST["name"] . "' and email='" . $_POST["email"] . "'";
        $stmt = $pdo->query($sql);
        if ($row = $stmt->fetch() > 0) {redirect("signup.php?msg=Email already Exists");}
...
$sql = "INSERT INTO login (email,uname,password,wardemail,address,phone,pincode) VALUES (?,?,?,?,?,?,?)";
		try {
			...
			if($ins){redirect("login.php?msg=Registered Successfully");}
  ...
  <body>	
	...
	<section class="contact-page">
        <div  class="container">
            ...
            <tr><th><label>Email-ID *</label></th>
            <td><input type="email" data-validation="email" name="email"></td></tr>

            <tr><th><label>Ward's JPIS Email-ID *</label></th>
            <td><input type="email" data-validation="email" name="wEmail"></td></tr>

            <tr><th><label>Phone Number *</label></th>
            <td><input type="phone" data-validation="length" data-validation-length="10-12" name="phone"></td></tr>

            <tr><th><label>Pincode *</label></th>
            <td><input type="phone" data-validation="length" data-validation-length="10-12" name="pincode"></td></tr>

            <tr><th><label>Password *</label></th>
            <td><input name="password" type="password" data-validation="length" data-validation-length="min8">
                <br><label style="font-size: 9px">Password must be minimum 8 characters in length.<br>And should also include a numeric digit in it. </label></td></tr>

            <tr><th><label>Confirm Password *</label></th>
<td><input type="password" data-validation="confirmation" name="repeat" data-validation-confirm="password"></td></tr>

            <tr><th><label>Address *</label></th>
            <td><input type="text" name="address"></td></tr>

            
            <tr><th style=" ;float:right">
